...
[-d genome Directory] [-t Threads] [-g Group prefix] [-o Output directory] -- RunOrthoMCL
    ...
    -d <dir> Directory containing your genome sequences
...
command -v orthomclInstallSchema >/dev/null 2>&1 || { printf "orthoMCL is not installed or is not in the PATH. Aborting\n" >&2; exit 1; }
command -v blastp >/dev/null 2>&1 || { printf "blastp is not installed or is not in the PATH. Aborting.\n" >&2; exit 1; }
command -v mcl >/dev/null 2>&1 || { printf "mcl is not installed or is not in the PATH. Aborting\n" >&2; exit 1; }
command -v perl >/dev/null 2>&1 || { printf "perl is not installed or is not in the PATH. Aborting\n" >&2; exit 1; }
command -v python >/dev/null 2>&1 || { printf "python is not installed or is not in the PATH. Aborting\n" >&2; exit 1; }
perl -MDBI -e ";" >/dev/null 2>&1 || { printf "the DBI libraries are not installed. Aborting\n" >&2; exit 1; }
...
while IFS=$',' read genome format code field
  do
    if [ "$format" == "genbank" ]
      then
        ...
        if [ "$?" != 0 ]
          then
            echo "orthomclAdjustFasta failed. Please check your GenomeList. Aborting." >&2
            exit 1
        fi
    elif [ "$format" == "fasta" ]
      then
      ...
      echo "Format: $format not recognized. Please check your GenomeList. Aborting." >&2
      exit 1
    fi
    cd "$WorkDir"
  done <"${GenomeList}"
